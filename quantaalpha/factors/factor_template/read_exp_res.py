...
# ------------------------------------------------------------------
# Market-aware qlib.init
#
# rdagent's bundled read_exp_res.py calls qlib.init() with zero args,
# which always falls back to the A-share (cn_data) provider. That makes
# the result-extraction step read from the wrong market's experiment
# recorder when mining crypto factors.
#
# Fix: pick provider_uri from PROVIDER_URI env (injected by
# workspace.execute()), fall back to the A-share path when absent.
# ------------------------------------------------------------------
provider_from_env = os.environ.get("PROVIDER_URI")
qlib.init(provider_uri=provider_from_env)
